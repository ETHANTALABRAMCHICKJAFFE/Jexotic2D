...
//		    }
	...
	*/
	...
	protected void paintComponent(Graphics g) {
		...
		for (int i = 0; i < gameObjects.size(); i++) {
			...
			if(gameObjects.get(i) == inspector.gameObject){
				...
				g2.draw(new Rectangle2D.Double(s.getReferencePoint().getX()-(newWidth-width), s.getReferencePoint().getY()-(newHeight-height), newWidth, newHeight));
	...
	public void addCircle(){
		...
		GameObject Circle = new GameObject(position, new Vector2d(-3*Math.random()+5,Math.random()*10), Math.random()*10+1, Color.red, c);
...
//		Circle.addScriptFile("DemoScript1.java");
		...
		//private Shape rect = new Rectangle2D.Double(position.getX(), position.getY(), =, 100);
//	    Shape circleShape = new Ellipse2D.Double(Circle.getPosition().getX(), Circle.getPosition().getY(), radius,radius);
...
//		double radius = 20;
	...
	public boolean checkIfMouseIsWithinAGameObject(MouseEvent e){
		...
		for (int i = 0; i < gameObjects.size(); i++) {
			...
			Vector2d corrected = new Vector2d(c.getReferencePoint().getX()+c.getRadius(),c.getReferencePoint().getY()+c.getRadius()*4);
			...
			if(g.getCollider().getColliderShape() instanceof Rectangle){
				...
				Vector2d AB = r.topSideVector,BC = r.rightSideVector,AM = Vector2d.sub(r.topLeftPoint, mousePos),BM = Vector2d.sub(r.topRightPoint, mousePos);
